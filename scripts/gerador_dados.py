...
def gerar_item_pedido(produtos, quantidade_pedidos) -> dict:
	...
	id_pedido = random.randint(1, quantidade_pedidos)
...
quantidade_itens = int(input("Quantos itens deseja gerar?: "))

while quantidade_itens <= 0:
	print("Número inválido!")
	quantidade_itens = int(input("Digite novamente, quantos itens deseja gerar?: "))
...
with open("inserts.sql", "w", encoding="utf-8") as arquivo:
	...
	while contador <= quantidade_itens:
		item_pedido = gerar_item_pedido(produtos, quantidade_pedidos)
		sql = gerar_insert_item(item_pedido)
		if contador < quantidade_itens:
			arquivo.write(sql + ",\n")
		else:
			arquivo.write(sql + ";")
...
print(f"{quantidade_clientes} clientes, {quantidade_produtos} produtos, {quantidade_pedidos} pedidos e {quantidade_itens} itens gerados com sucesso!")
